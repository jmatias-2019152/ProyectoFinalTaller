...
export const clientePut = async (req, res) => {
    ...
    try {
        ...
        if (clienteExistente.correo === correoActualizado) {
            const clienteActualizadoDB = await Cliente.findOneAndUpdate({ _id: uid, correo: correoActualizado, estado: true }, clienteActualizado, { new: true });
            global.tokenAcces = '';
            res.status(200).json({
            ...
            res.status(403).json({
                msg: "No tienes permisos para editar esta cuenta"
            });
        }
    } catch (error) {
